...
outputDebugString ( "MTA:DayZ Simplified | ss loaded" )
...
if data == "food" then
	if itemName == Item_food_burger then
		blood = 300
	elseif itemName == Item_food_pizza then
		blood = 300
	elseif itemName == Item_food_coockedmeat then
		blood = 1200
	elseif itemName == Item_food_beanscan then
		blood = 200
	elseif itemName == Item_food_pastacan then
		blood = 200
	end
...
elseif data == "thirst" then
	...
	if itemName == Item_food_waterbottle then
...
setTimer( function ()
	...
	elseif itemName == Item_medic_medickit then
		addPlayerStats (playersource,"blood",7000)
		setElementData(playersource,"bleeding",0)
		setElementData(playersource,itemName,getElementData(playersource,itemName)-1)
	elseif itemName == Item_medic_heatpack then
		setElementData(playersource,"cold",false)
		setElementData(playersource,"temperature",37)
		setElementData(playersource,itemName,getElementData(playersource,itemName)-1)
	elseif itemName == Item_medic_painkiller then
		setElementData(playersource,"pain",false)
		setElementData(playersource,itemName,getElementData(playersource,itemName)-1)
	elseif itemName == Item_medic_morphine then
		setElementData(playersource,"brokenbone",false)
		setElementData(playersource,itemName,getElementData(playersource,itemName)-1)
	elseif itemName == Item_medic_bloodbag then
...
setTimer( function ()
	if itemName == "bandage" then
		...
		setElementData(playersource,Item_medic_bandage,getElementData(playersource,Item_medic_bandage)-1)
	...
	elseif itemName == "giveblood" then
		...
		setElementData(playersource,Item_medic_bloodbag,getElementData(playersource,Item_medic_bloodbag)-1)
...
skinTable = {
{Item_skin_camo,287},
{Item_skin_civilian,179},
{Item_skin_ghilliesuit,285},
{Item_skin_Survivor,73},
}
...
function onPlayerRefillWaterBottle (itemName)
	if isElementInWater(source) then
		setElementData(source,Item_food_waterbottle,getElementData(source,Item_food_waterbottle)+1)
		setElementData(source,itemName,getElementData(source,itemName)-1)
		triggerClientEvent(source,"refreshInventoryManual",source)
		triggerClientEvent (source, "displayClientInfo", source,Item_food_waterbottle,shownInfos["filledup"],22,255,0)
	else
		triggerClientEvent (source, "displayClientInfo", source,Item_food_waterbottle,shownInfos["needwatersource"],255,22,0)
	end	
end
...
function onPlayerPitchATent (itemName)
 ...
 attachElements(tent,source,0,4,-1,0,0,180)
 mark = createMarker ( x, y, z-3, "cylinder", 3.5, 255, 0, 0, 105, source )
 setElementData(mark,"tentMark",true)
 setElementData(mark,"tent",tent)
 setElementData(mark,"tentCol",tentCol)
 outputChatBox("Press 'enter' to set up a tent.",source,255,255,255)
 bindKey(source,"enter","down",setTent,tent,vrot,mark)
            triggerClientEvent(source,"refreshInventoryManual",source)
...
function setTent(player, key, keyState, tent, vrot, mark)
    if isElement(tent) then
        detachElements(tent,player)
 destroyElement(mark)
        prot = getPedRotation(player)
        local vrot = prot+180
        setElementRotation(tent,0,0,vrot)
        unbindKey(player,"enter","down",setTent)
    else
        unbindKey(player,"enter","down",setTent)
    end
end
...
function addPlayerCookMeat ()
	...
	setTimer(function()
			...
			setElementData(playersource,Item_food_coockedmeat,getElementData(playersource,Item_food_coockedmeat)+meat)
...
function playerRadioChat(playersource,cmd,...)
	if cmd == "radiochat" then
		...
		if (getElementData(playersource,Item_toolbelt_radiodevice) or 0) <= 0 then outputChatBox(shownInfos["noradio"],playersource) return end
        local posX, posY, posZ = getElementPosition( playersource )
        local chatSphere = createColSphere( posX, posY, posZ, chatEadioRadius )
        local nearbyPlayers = getElementsWithinColShape( chatSphere, "player" )
        destroyElement( chatSphere )
        for index, nearbyPlayer in ipairs( nearbyPlayers ) do
			if getElementData(nearbyPlayer,Item_toolbelt_radiodevice) > 0 then
...
outputDebugString ( "MTA:DayZ Simplified | ss end" )
