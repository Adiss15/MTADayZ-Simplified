...
outputDebugString ( "MTA:DayZ Simplified | menu-client loaded" )
...
spalteGuiImage[1] = guiCreateStaticImage(0,0.45,0.1,0.025,":DayZ/images/scrollmenu_1.png",true)
spalteGuiImage[2] = guiCreateStaticImage(0,0.475,0.1,0.025,":DayZ/images/scrollmenu_1.png",true)
spalteGuiImage[3] = guiCreateStaticImage(0,0.5,0.1,0.025,":DayZ/images/scrollmenu_1.png",true)
spalteGuiImage[4] = guiCreateStaticImage(0,0.525,0.1,0.025,":DayZ/images/scrollmenu_1.png",true)
...
if getElementData(source,"parent") == getLocalPlayer() then return end
		...
		if getElementData(source,"patrolstation") then
			...
			setElementData(getLocalPlayer(),"currentCol",source)
			setElementData(getLocalPlayer(),"loot",false)
			setNewbieInfo (true,"patrolstation",languageTextTable[languageCode]["patrolstation"],source)
			return
		end
		if getElementData(source,"wirefence") then
			...
			setElementData(getLocalPlayer(),"currentCol",source)
			setElementData(getLocalPlayer(),"loot",false)
			setNewbieInfo (true,"Wirefence",languageTextTable[languageCode]["wirefence"],source)
			return
		end
		if getElementData(source,"fireplace") then
			...
			setElementData(getLocalPlayer(),"currentCol",source)
			setElementData(getLocalPlayer(),"loot",false)
			setNewbieInfo (true,"Fireplace",languageTextTable[languageCode]["fireplace"],source)
			isInFirePlace = true
			return
		end
		if getElementData(source,"deadman") then
			...
			setNewbieInfo (true,"Gear",languageTextTable[languageCode]["deadman"],source)
			return
		end
		if getElementData(source,"item") then
			...
			setElementData(getLocalPlayer(),"currentCol",source)
			setElementData(getLocalPlayer(),"loot",false)
			setNewbieInfo (true,"Item pickup",languageTextTable[languageCode]["item"],source)
			return
		end
		if getElementData(source,"helicrash") then
			...
			setNewbieInfo (true,"Gear",languageTextTable[languageCode]["helicrash"],source)
			return
		end
		if getElementData(source,"hospitalbox") then
			...
			setNewbieInfo (true,"Gear",languageTextTable[languageCode]["hospitalbox"],source)
			return
		end
		if getElementData(source,"vehicle") then
			...
			setNewbieInfo (true,"Gear",languageTextTable[languageCode]["vehicle"],source)
			return
			end
		end
		if getElementData(source,"itemloot") then
			...
			setElementData(getLocalPlayer(),"currentCol",source)
			setNewbieInfo (true,"Gear",languageTextTable[languageCode]["itemloot"],source)
			return
		end
...
function()
	...
	local x,y,z = getScreenFromWorldPosition (x,y,z)
...
if itemName == "tent" then
 ...
 showCursor(true)
 disableMenu()
 tentGuiConfirm = guiCreateWindow((screenW - 254) / 2, (screenH - 147) / 2, 254, 147, "Tent", false)
 guiWindowSetMovable(tentGuiConfirm, false)
 guiWindowSetSizable(tentGuiConfirm, false)
 tentLabelConfirm = guiCreateLabel(23, 34, 207, 15,languageTextTable[languageCode]["tentremove"], false, tentGuiConfirm)
 guiSetFont(tentLabelConfirm, "default-bold-small")
 tentYesConfirm = guiCreateButton(23, 64, 81, 51, languageTextTable[languageCode]["tentremovebutton"], false, tentGuiConfirm)
 tentCancelConfirm = guiCreateButton(149, 64, 81, 51, languageTextTable[languageCode]["tentcancelbutton"], false, tentGuiConfirm)    
 addEventHandler("onClientGUIClick",tentYesConfirm,
 function ()
 local col = getElementData(getLocalPlayer(),"currentCol")
 showCursor(false)
 guiSetVisible(tentGuiConfirm, false)
 triggerServerEvent("removeTent",getLocalPlayer(),getElementData(col,"parent"))
 end, false)
 addEventHandler("onClientGUIClick",tentCancelConfirm,
 function ()
 showCursor(false)
 guiSetVisible(tentGuiConfirm, false)
 end, false)
 return
		...
		if itemName == "patrolstation" then
			local col = getElementData(getLocalPlayer(),"currentCol")
				...
				triggerEvent ("displayClientInfo",getLocalPlayer(),"patrolstation",languageTextTable[languageCode]["FullGasCanister"],22,255,0)
				disableMenu()
			return
		end
			...
			if getPlayerCurrentSlots() + getItemSlots(itemName) <= getPlayerMaxAviableSlots() then	
				...
				startRollMessage2("Inventory", languageTextTable[languageCode]["mensageinventory"], 255, 22, 0 )
			end
	end
end
...
outputDebugString ( "MTA:DayZ Simplified | menu-client end" )
